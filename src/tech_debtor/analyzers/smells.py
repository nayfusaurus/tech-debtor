    ...
    def analyze(self, file_path: str, source: str, tree: Tree, config: Config) -> list[Finding]:
        ...
        for func in functions:
            ...
            if length > config.max_function_length:
                ...
                findings.append(Finding(
                    ...
                    remediation_minutes=max(5, excess * 2), symbol=name,
                ))
            ...
            if body:
                ...
                if depth > config.max_nesting_depth:
                    findings.append(Finding(
                        ...
                        remediation_minutes=max(5, (depth - config.max_nesting_depth) * 10), symbol=name,
                    ))
            ...
            if param_count > config.max_parameters:
                findings.append(Finding(
                    file_path=file_path, line=line, end_line=end_line,
                    debt_type=DebtType.SMELL, severity=Severity.MEDIUM,
                    ...
                    remediation_minutes=max(5, (param_count - config.max_parameters) * 5), symbol=name,
                ))
        ...
        for cls in classes:
            ...
            if method_count > GOD_CLASS_METHOD_THRESHOLD:
                findings.append(Finding(
                    file_path=file_path, line=cls.start_point[0] + 1, end_line=cls.end_point[0] + 1,
                    debt_type=DebtType.SMELL, severity=Severity.HIGH,
                    ...
                    remediation_minutes=method_count * 5, symbol=cls_name,
                ))
